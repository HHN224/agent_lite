...
面向终端 agent 的可迁移离线题包：**6 类可执行题、3 档规模、确定性数据生成、客观评分、阶段快照和实验方案**。主要测工作成果是否准确，以及多阶段任务能否持续完成。

这不是 agent-lite 的既有成绩，也不是官方 benchmark。自测只验证题包与评分器。实际准确率、耗时和成本必须运行被测 agent 后填写。

## 从哪里开始

1. 阅读 [题目册](docs/TASKS.md)，了解每题的输入、难点、交付和验收。
...
`smoke / standard / long` 是同一题族的规模档，不是 18 个独立能力。随机种子也不等于新的题族。标准 L01 为 8 阶段，长档为 16 阶段；做得快不扣分，不用无意义的步骤数强行拉长时间。

## 环境与迁移

复制整个 `agent_eval/` 到任意目录即可。生成器、评分器和自测只依赖 **Python 3.10+ 标准库**，不需要 agent-lite、模型密钥、网络或第三方库；不修改原项目运行时代码。agent-lite 本身的启动环境仍按原项目要求配置。
...
以上命令使用现有模型配置，会在你实际运行时产生模型调用；本题包不自动调用模型，也未替你运行付费评测。`auto` 仅适用于已准备好的专用评测环境。其他 agent 只要能读写 workspace、执行 Python，就能用同一题面和评分器。
...
v1 范围：本地数据/配置、Python 行为、多阶段文件交付；没有浏览器视觉、联网搜索、并发竞争、真正的分布式事务评分。扩展题仍是设计稿，未计入六类可运行题。本题包测试通过不意味着测得 agent 的任何成功率。
