...
**压缩失败不会拖死任务**（真实事故：328 条消息的会话里压缩 0 次成功，`last_error=APITimeoutError`，任务卡住）：

- **根因是输出额度给少了**：这个路由写正文前会先产出几千 token 的 reasoning，而 `--compact-max-tokens` 一度被当成 API 的硬输出上限（默认 2000）→ 额度全烧在思考上、正文一个字写不出、服务端返回 `finish_reason=length`。用事故会话的真实区间实测：**4/4 次失败**；只把额度提到 8000 就立刻成功。现在两者分开：该参数只作提示词里的目标长度，API 上限自动取 `max(值×4, 6000)`。`finish_reason=length` 时若已流出正文，也保留截断的摘要而不是整条丢弃。
- **失败退避**：压缩失败后计量仍然保留，但会记录「再增长约 20 条消息才重试」；连续失败到 3 次就不再空转。修前是每个工具轮次都重发一次摘要请求（实测一个任务里被重试 7 次）。
- **救回被污染的摘要**：这个路由会把工具调用序列化成 DSML 文本混进正文，那种输出往往是「一段真摘要 + 一段工具调用尾巴」；现在只砍掉标记及其之后的内容（`sanitize_summary`），而不是整条丢弃。
- **机械兜底**（`--compact-fallback digest`，默认）：模型摘要彻底压不动时，用不依赖模型的机械摘要继续压——抽取用户目标、触及的文件、执行过的命令、最近结论，明确标注 `[机械压缩摘要]`。实测同一段真实历史：payload 3.2M 字符 → 92K 字符。要恢复严格档（彻底停手、只留原文）用 `--compact-fallback none`。
- 退避与兜底状态写进 `session.runtime_status["compaction"]`，CLI / TUI 会明确提示「压缩未生效（连续第 N 次）」或「压缩已停手」；用户手动 `/compact` 走 `force`，能越过退避与停手真正再试一次。
